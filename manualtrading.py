...
def find_best_manual_trade(trades,start,amount):
    ...
    for i in range(4):
        ...
        for j in range(4):
            ...
            items = items_1 * trades[names[i]][j]
            ...
            for k in range(4):
                ...
                items = items_1 * trades[names[j]][k]

                #if(liquidate(items,names[k])>best_amount):
                #    best_amount = liquidate(items,names[k])
                #    best_trades = manual_trades

                for l in range(4):
                    manual_trades = [start + " to " + names[i], names[i] + " to " + names[j], names[j] + " to " + names[k]]
                    manual_trades.append(names[k] + " to " + names[l])
                    items = items_1 * trades[names[k]][l]

                    if(liquidate(items,names[l])>best_amount):
                        best_amount = liquidate(items,names[l])
                        best_trades = manual_trades + [names[k] + " to " + "shells"]
